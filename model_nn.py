...
b1 = init_random([config.nn_units_h1], config.std_dev, "b1", "Bias 1 Summary")

w2 = init_random([config.nn_units_h1, config.nn_units_h2], config.std_dev, "w2", "Weight 2 Summary")
b2 = init_random([config.nn_units_h2], config.std_dev, "b2", "Bias 2 Summary")

wo = init_random([config.nn_units_h2, config.num_classes], config.std_dev, "w3", "Output Summary")
...
with tf.name_scope("cost_function"):
    ...
    train_step = tf.train.AdamOptimizer(config.nn_learning_rate).minimize(cross_entropy)
